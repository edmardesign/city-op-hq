...
interface CityOption { id: number; nome: string }
...
function openCleanWhatsApp(role: Role, city: string) {
  ...
  window.location.assign(`https://wa.me/${WHATSAPP_DIGITS}?text=${encodeURIComponent(messages[role])}`);
}
...
export function DriverDeliveryLeadDialog({ title, description }: Props) {
  ...
  function validateStep() {
    if (step === "location") return values.state && values.city ? "" : "Escolha o estado e a cidade.";
  ...
  async function continueFlow(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    if (stepIndex < steps.length - 1) { setStepIndex((current) => current + 1); return; }

    setIsSubmitting(true);
    try {
      const result = await submitLead({ data: {
        state: values.state ?? "", city: values.city ?? "", role: values.role as Role,
        name: values.name ?? "", phone: values.phone ?? "", email: values.email ?? "",
        instagram: values.instagram, ...getTracking(),
      } });
  ...
  function handleOpenChange(nextOpen: boolean) {
    ...
    if (!nextOpen) window.setTimeout(() => {
      setStepIndex(0); setValues({}); setCities([]); setCitiesStatus("idle"); setError("");
    }, 200);
  }
      ...
      <DialogContent className="inset-x-0 bottom-0 top-auto max-h-[92dvh] w-full max-w-none translate-x-0 translate-y-0 overflow-y-auto rounded-t-2xl border-x-0 border-b-0 p-0 sm:left-1/2 sm:top-1/2 sm:max-w-xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl sm:border">
        <div className="p-6 sm:p-10">
          <DialogHeader className="pr-8 text-left">
            <div className="mb-7 flex items-center gap-4"><span className="text-xs font-semibold text-muted-foreground">{stepIndex + 1} de {steps.length}</span><Progress value={((stepIndex + 1) / steps.length) * 100} className="h-1" /></div>
            ...
            <DialogDescription className="pt-2 text-sm leading-6">{step === "instagram" ? "Opcional. Você pode deixar em branco." : description}</DialogDescription>
          </DialogHeader>
          <form onSubmit={continueFlow} className="mt-8">
            <div key={step} className="animate-fade-in">
              {step === "location" && <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="mb-2 block text-sm font-semibold" htmlFor="driver-state">Estado</label><Select value={values.state ?? ""} onValueChange={loadCities}><SelectTrigger id="driver-state" className="h-14 rounded-xl px-4 text-base"><SelectValue placeholder="Selecione o estado" /></SelectTrigger><SelectContent>{BRAZILIAN_STATES.map((state) => <SelectItem key={state.uf} value={state.uf}>{state.name}</SelectItem>)}</SelectContent></Select></div>
                <div><label className="mb-2 block text-sm font-semibold" htmlFor="driver-city">Cidade</label><Select value={values.city ?? ""} onValueChange={(city) => { setValues((current) => ({ ...current, city })); setError(""); }} disabled={!values.state || citiesStatus !== "ready"}><SelectTrigger id="driver-city" className="h-14 rounded-xl px-4 text-base"><SelectValue placeholder={citiesStatus === "loading" ? "Carregando..." : "Selecione a cidade"} /></SelectTrigger><SelectContent>{cities.map((city) => <SelectItem key={city.id} value={city.nome}>{city.nome}</SelectItem>)}</SelectContent></Select></div>
              ...
              {step === "role" && <RadioGroup value={values.role ?? ""} onValueChange={(role) => { setValues((current) => ({ ...current, role })); setError(""); }} className="gap-3">{roleOptions.map((option) => <label key={option.value} className={cn("flex cursor-pointer items-center gap-3 rounded-xl border p-4 text-sm transition-all", values.role === option.value ? "border-primary bg-primary/10" : "border-border hover:border-primary/60")}><RadioGroupItem value={option.value} /><span>{option.label}</span></label>)}</RadioGroup>}
              {(step === "name" || step === "phone" || step === "email" || step === "instagram") && <Input value={values[step] ?? ""} onChange={(event) => { setValues((current) => ({ ...current, [step]: event.target.value })); setError(""); }} type={step === "email" ? "email" : step === "phone" ? "tel" : "text"} inputMode={step === "email" ? "email" : step === "phone" ? "tel" : "text"} autoComplete={step === "name" ? "name" : step === "phone" ? "tel" : step === "email" ? "email" : "off"} placeholder={step === "name" ? "Nome completo" : step === "phone" ? "(75) 99999-9999" : step === "email" ? "voce@email.com" : "@seuperfil"} maxLength={step === "email" ? 255 : 120} aria-invalid={Boolean(error)} aria-describedby={error ? "driver-lead-error" : undefined} className="h-14 rounded-xl px-4 text-base shadow-none" />}
            </div>
            <div className="min-h-7 pt-2">{error && <p id="driver-lead-error" className="text-sm text-destructive" role="alert">{error}</p>}</div>
            <div className="mt-4 flex items-center justify-between gap-3">
              ...
              <Button type="submit" size="lg" disabled={isSubmitting || citiesStatus === "loading"} className="h-12 min-w-36 rounded-xl px-6 font-bold">{isSubmitting ? "Enviando..." : stepIndex === steps.length - 1 ? "Enviar cadastro" : "Continuar"}{stepIndex === steps.length - 1 ? <Check aria-hidden="true" /> : <ArrowRight aria-hidden="true" />}</Button>
            </div>
          </form>
        </div>
      </DialogContent>
