...
import { ProgressiveLeadDialog, OPEN_LEAD_DIALOG_EVENT, type ProgressiveLeadConfig } from "@/components/progressive-lead-dialog";
...
export function openWhatsApp(message: string) {
  ...
  window.location.assign(`https://wa.me/${WHATSAPP_DIGITS}?text=${encodeURIComponent(trackedMessage)}`);
}
...
export function CampaignHero({ eyebrow, title, description, cta, image, imageAlt, proof = [], imagePosition }: CampaignHeroProps) {
  return (
    <section className="relative isolate bg-brand-black pb-12 pt-28 text-brand-white md:pb-20 md:pt-36">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 md:px-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
        <div className="reveal-up max-w-2xl">
          ...
          <h1 className="mt-5 text-5xl font-bold leading-[1.02] sm:text-6xl lg:text-7xl">{title}</h1>
          <p className="mt-6 max-w-xl text-base leading-7 text-brand-white/70 md:text-lg md:leading-8">{description}</p>
          <Button onClick={openLeadDialog} size="lg" className="mt-8 h-14 w-full rounded-xl px-7 text-sm font-bold sm:w-auto">
            {cta}<ArrowRight aria-hidden="true" />
          </Button>
          {proof.length > 0 && <p className="mt-5 text-xs leading-5 text-brand-white/45">{proof.join(" • ")}</p>}
        </div>
...
export function FeatureGrid({ items, columns = 3, dark = false }: { items: FeatureItem[]; columns?: 3 | 4 | 6; dark?: boolean }) {
  return (
    <div className={cn("grid gap-3", columns === 3 && "md:grid-cols-3", columns === 4 && "md:grid-cols-2 lg:grid-cols-4", columns === 6 && "grid-cols-2 md:grid-cols-3 lg:grid-cols-6")}>
      {items.map((item) => (
        <article key={item.title} className={cn("rounded-xl border p-5 transition-transform duration-300 hover:-translate-y-1", dark ? "border-brand-white/10 bg-brand-white/5" : "border-border bg-card")}>
          {item.icon && <div className="mb-7 text-primary">{item.icon}</div>}
          <h3 className={cn("text-base font-bold md:text-lg", dark && "text-brand-white")}>{item.title}</h3>
...
export function ConversionSection({ title, description, cta }: { title: string; description: string; cta: string }) {
  return (
    <section id="conversao" className="bg-brand-surface py-20 md:py-28">
      <div className="mx-auto flex max-w-4xl flex-col items-start justify-between gap-8 px-5 md:flex-row md:items-end md:px-8">
        ...
        <Button onClick={openLeadDialog} size="lg" className="h-14 w-full shrink-0 rounded-xl px-7 font-bold md:w-auto">{cta}<ArrowRight aria-hidden="true" /></Button>
      </div>
    </section>
  );
}
...
export function MediaBand({ image, alt, eyebrow, title, description, reverse = false }: { image: string; alt: string; eyebrow: string; title: ReactNode; description: string; reverse?: boolean }) {
  return (
    <section className="py-20 md:py-28">
      <div className={cn("mx-auto grid max-w-7xl gap-12 px-5 md:px-8 lg:grid-cols-2 lg:items-center", reverse && "lg:[&>*:first-child]:order-2")}>
        <div className="overflow-hidden rounded-2xl bg-muted"><img src={image} alt={alt} loading="lazy" width={768} height={960} className="aspect-[4/5] w-full object-cover" /></div>
