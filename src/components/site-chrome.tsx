  ...
  return (
    <header className="absolute inset-x-0 top-0 z-40 border-b border-brand-white/10">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 md:px-8">
        <Link to="/" aria-label="Bora Zé — início" className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
    ...
    <footer className="border-t border-border bg-brand-black py-10 text-brand-white">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-5 md:px-8">
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          ...
          <nav aria-label="Outras oportunidades Bora Zé" className="flex flex-wrap gap-x-5 gap-y-3 text-xs text-brand-white/45">
            {links.map((item) => (
              <Link key={item.to} to={item.to} className="transition-colors hover:text-brand-white focus-visible:text-brand-white">
                {item.label}
              </Link>
            ))}
          </nav>
        </div>
        <p className="text-xs text-brand-white/35">© {new Date().getFullYear()} Bora Zé. Todos os direitos reservados.</p>
      </div>
    </footer>
  );
}